...
public class xvdfdd {
	...
	String parentwindaddd=driver.getWindowHandle();
	...
	if(!parentwindaddd.equals(Childwindadd)) {
		driver.switchTo().window(Childwindadd);
		WebElement addtoCart=driver.findElement(By.xpath("//input[@name='submit.buy-now']"));
JavascriptExecutor js=(JavascriptExecutor) driver;
js.executeScript("argument[0].ScrollIntoView",addtoCart);
		driver.close();
	}
	}
	
	driver.switchTo().window(parentwindaddd);
	driver.close();
